"""
Enhanced dictionary processing using CAMeL Tools.
This script will enrich your existing dictionary with advanced morphological analysis.
"""
import sqlite3
import json
import logging
...
from services.camel_final import camel_processor, enhance_dictionary_entry

logging.basicConfig(level=logging.INFO)
logger = logging.getLogger(__name__)

class CamelDictionaryEnhancer:
    ...
    def __init__(self, db_path: str):
        ...
        self.error_count = 0
        self.camel_available = camel_processor.available
    
    def enhance_all_entries(self, batch_size: int = 50, max_entries: int = None):
        """
        ...
        while offset < total_entries:
            ...
            # Get batch of entries that haven't been processed yet
            cursor.execute("""
                SELECT id, lemma, root, pos, pattern, lemma_norm, 
                       subpos, register, domain
                FROM entries 
    ...
    def _enhance_entry(self, cursor, entry):
        ...
        (entry_id, lemma, root, pos, pattern, lemma_norm, 
         subpos, register, domain) = entry
        
        # Use lemma as the word to analyze
        word_to_analyze = lemma or lemma_norm
        ...
        if camel_analysis.get("morphology"):
            confidence = min(1.0, len(camel_analysis["morphology"]) / 10.0)
        ...
        # Update main fields if CAMeL provides better data and current fields are empty
        updates = []
        params = []
    ...
    def generate_statistics(self):
        ...
        cursor.execute("""
            SELECT lemma, camel_lemmas, camel_roots, camel_pos_tags
            FROM entries 
        ...
        if sample_entries:
            print("\nSample enhanced entries:")
            for lemma, lemmas_json, roots_json, pos_json in sample_entries:
                try:
                    lemmas = json.loads(lemmas_json)
                    roots = json.loads(roots_json)
                    pos_tags = json.loads(pos_json)
                    print(f"  {lemma} → lemmas: {lemmas[:2]}, roots: {roots}, POS: {pos_tags}")
                except:
                    print(f"  {lemma} → (parsing error)")

def test_camel_enhancement():
    """Test CAMeL Tools enhancement on a few sample words."""
    logger.info("Testing CAMeL Tools enhancement...")
    
    if not camel_processor.available:
        logger.error("CAMeL Tools not available!")
        return False
    
    test_words = ["كتاب", "يكتب", "مكتبة", "الكتاب", "كاتب", "بيت", "ماء", "قلم"]
    
    print("\n=== CAMeL Tools Enhancement Test ===")
    for word in test_words:
        print(f"\n--- Enhancing: {word} ---")
        
        # Test basic analysis
        analysis = camel_processor.analyze_word(word)
        print(f"Normalized: {analysis.get('normalized')}")
        print(f"Lemmas: {analysis.get('possible_lemmas', [])}")
        print(f"Roots: {analysis.get('roots', [])}")
        print(f"POS: {analysis.get('pos_tags', [])}")
        
        # Test enhancement
        enhanced = enhance_dictionary_entry(word, {"original_lemma": word})
        print(f"Enhanced lemma: {enhanced.get('lemma')}")
        print(f"Enhanced root: {enhanced.get('root')}")
        print(f"Enhanced POS: {enhanced.get('pos')}")
    
    logger.info("CAMeL Tools enhancement test completed successfully!")
    return True
...
if __name__ == "__main__":
    ...
    if args.test:
        test_camel_enhancement()
    
    if args.enhance:
        enhancer = CamelDictionaryEnhancer(args.db)
