"""
CAMeL Tools integration for Arabic dictionary enhancement.
Provides advanced morphological analysis, dialectal processing, and NLP features.
"""
import json
import logging
...
try:
    ...
    logging.warning(f"CAMeL Tools not available: {e}")
    CAMEL_AVAILABLE = False

logger = logging.getLogger(__name__)

class CamelProcessor:
    """Advanced Arabic text processor using CAMeL Tools."""
    
    def __init__(self):
        self.available = CAMEL_AVAILABLE
        if not self.available:
            logger.warning("CAMeL Tools not available. Advanced features disabled.")
            return
            
        try:
            # Initialize core components
            self._init_morphology()
            self._init_dialect_id()
    ...
    def _init_morphology(self):
        ...
        try:
            # Use CALIMA MSA database for morphological analysis
    ...
    def analyze_word(self, word: str) -> Dict[str, Any]:
        ...
        Returns:
            Dictionary with morphological analysis, dialect info, etc.
        """
        if not self.available:
        ...
        if self.dialect_id:
            try:
                ...
                logger.warning(f"Dialect identification failed for '{word}': {e}")
        ...
        if self.sentiment:
            try:
                ...
                logger.warning(f"Sentiment analysis failed for '{word}': {e}")
        ...
        if self.ner:
            try:
                ...
                logger.warning(f"NER failed for '{word}': {e}")
    ...
    def generate_forms(self, lemma: str, target_features: Dict[str, str]) -> List[str]:
        ...
        Args:
            lemma: Base lemma
            target_features: Dictionary of morphological features
        ...
        try:
            ...
            return [form['diac'] for form in generated]
        except Exception as e:
    ...
    def identify_dialect(self, text: str) -> Optional[Dict[str, Any]]:
        """Identify the dialect of Arabic text."""
        if not self.available or not self.dialect_id:
            return None
        
        try:
            result = self.dialect_id.predict([text])
            if result:
                return {
                    "top_dialect": result[0].top,
                    "confidence": result[0].scores[result[0].top],
                    "all_scores": dict(result[0].scores)
                }
        except Exception as e:
            logger.warning(f"Dialect identification failed: {e}")
        
        return None
    
    def analyze_sentiment(self, text: str) -> Optional[Dict[str, Any]]:
        """Analyze sentiment of Arabic text."""
        if not self.available or not self.sentiment:
            return None
        
        try:
            result = self.sentiment.predict([text])
            if result:
                return {
                    "sentiment": result[0].top,
                    "confidence": result[0].scores[result[0].top],
                    "all_scores": dict(result[0].scores)
                }
        except Exception as e:
    ...
    def extract_entities(self, text: str) -> List[Dict[str, str]]:
        ...
        if not self.available or not self.ner or not self.tokenizer:
            return []
        
        try:
            tokens = self.tokenizer.tokenize(text)
            entities = self.ner.predict(tokens)
            return [{"text": ent.text, "label": ent.label, "start": ent.start, "end": ent.end} 
                   for ent in entities]
        except Exception as e:
...
# Global instance
camel_processor = CamelProcessor()
